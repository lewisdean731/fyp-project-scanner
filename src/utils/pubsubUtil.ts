...
export async function publishMessage(id:string) {
  ...
  try {
    const messageId = await pubSubClient.topic(topicName).publish(data, attributes);
    console.log(`Message ${messageId} published for project ${id}`);
  } catch (error) {
    console.error(`Received error while publishing message for project ${id}: ${error.message}`);
  }
}
